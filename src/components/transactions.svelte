<div class="flex flex-col flex-auto overflow-hidden relative">
  <menu
    class="shrink-0 [min-block-size:fit-content] overflow-x-audo overflow-y-hidden overscroll-x-contain snap-x snap-mandatory"
  >
    <ul class="flex flex-row justify-between px-6 py-4">
      <li class="snap-start inline-flex items-center whitespace-nowrap">
        <a href="#incomming">
          <svg
            width="46"
            height="36"
            class="box-content"
            style="padding-left: 2px; padding-right: 12px"
            viewBox="0 0 46 36"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <rect
              x="11"
              y="1"
              width="34"
              height="34"
              rx="7"
              stroke="currentColor"
              stroke-width="2"
            />
            <path
              d="M22 22H0M22 22L15.6667 15M22 22L15.6667 29"
              stroke="currentColor"
              stroke-width="2"
            />
          </svg>
        </a>
      </li>
      <li class="snap-start inline-flex items-center whitespace-nowrap">
        <a href="#transactions">
          <svg
            width="58"
            height="36"
            class="box-content"
            style="padding-left: 2px;"
            viewBox="0 0 58 36"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <rect
              x="11"
              y="1"
              width="34"
              height="34"
              rx="7"
              stroke="currentColor"
              stroke-width="2"
            />
            <path
              d="M56 14H34M56 14L49.6667 7M56 14L49.6667 21"
              stroke="currentColor"
              stroke-width="2"
            />
            <path
              d="M22 22H0M22 22L15.6667 15M22 22L15.6667 29"
              stroke="currentColor"
              stroke-width="2"
            />
          </svg>
        </a>
      </li>
      <li class="snap-start inline-flex items-center whitespace-nowrap">
        <a href="#outgoing">
          <svg
            width="48"
            height="36"
            class="box-content"
            style="padding-left: 12px"
            viewBox="0 0 48 36"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <rect
              x="1"
              y="1"
              width="34"
              height="34"
              rx="7"
              stroke="currentColor"
              stroke-width="2"
            />
            <path
              d="M46 14H24M46 14L39.6667 7M46 14L39.6667 21"
              stroke="currentColor"
              stroke-width="2"
            />
          </svg>
        </a>
      </li>
      <li class="snap-start inline-flex items-center whitespace-nowrap">
        <a href="#statistics">
          <svg
            width="36"
            height="36"
            class="box-content"
            style="padding-left: 12px; padding-right: 12px"
            viewBox="0 0 36 36"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <rect
              x="1"
              y="1"
              width="34"
              height="34"
              rx="7"
              stroke="currentColor"
              stroke-width="2"
            />
            <path
              d="M7 29L14.5 15L21 21L29 7"
              stroke="currentColor"
              stroke-width="2"
              stroke-linecap="round"
            />
          </svg>
        </a>
      </li>
      <li class="snap-start inline-flex items-center whitespace-nowrap">
        <a href="#goals">
          <svg
            width="36"
            height="36"
            class="box-content"
            style="padding-left: 12px; padding-right: 12px"
            viewBox="0 0 36 36"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <rect
              x="1"
              y="1"
              width="34"
              height="34"
              rx="7"
              stroke="currentColor"
              stroke-width="2"
            />
            <circle cx="18" cy="18" r="13" stroke="currentColor" stroke-width="2" />
            <circle cx="18" cy="18" r="9" stroke="currentColor" stroke-width="2" />
            <circle cx="18" cy="18" r="5" stroke="currentColor" stroke-width="2" />
          </svg>
        </a>
      </li>
    </ul>
  </menu>
  <div
    class="[block-size:100%] grid grid-flow-col [grid-auto-columns:100%] overflow-x-audo overflow-y-hidden overscroll-x-contain snap-x snap-mandatory flex-grow gap-4"
  >
    <div
      id="incomming"
      class="rounded-t-3xl peppermint-green-bg-dark snap-start overflow-y-auto overscroll-y-contain"
    >
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming Incomming
      Incomming Incomming Incomming Incomming Incomming
    </div>
    <div
      id="transactions"
      class="rounded-t-3xl peppermint-green-bg-dark snap-start overflow-y-auto overscroll-y-contain"
    >
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions Transactions Transactions Transactions Transactions Transactions
      Transactions Transactions
    </div>
    <div
      id="outgoing"
      class="rounded-t-3xl peppermint-green-bg-dark snap-start overflow-y-auto overscroll-y-contain"
    >
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
      Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing Outgoing
    </div>
    <div
      id="statistics"
      class="rounded-t-3xl peppermint-green-bg-dark snap-start overflow-y-auto overscroll-y-contain"
    >
      Statistics Statistics Statistics Statistics Statistics Statistics Statistics Statistics
      Statistics Statistics Statistics Statistics Statistics Statistics Statistics Statistics
      Statistics Statistics Statistics Statistics Statistics Statistics Statistics Statistics
      Statistics Statistics Statistics Statistics Statistics Statistics Statistics Statistics
      Statistics Statistics Statistics Statistics Statistics Statistics Statistics Statistics
      Statistics Statistics Statistics Statistics Statistics Statistics Statistics Statistics
      Statistics Statistics Statistics Statistics Statistics Statistics Statistics Statistics
      Statistics Statistics Statistics Statistics Statistics Statistics Statistics Statistics
      Statistics Statistics Statistics
    </div>
    <div
      id="goals"
      class="rounded-t-3xl peppermint-green-bg-dark snap-start overflow-y-auto overscroll-y-contain"
    >
      Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals
      Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals
      Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals
      Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals
      Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals
      Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals
      Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals Goals
      Goals Goals Goals Goals Goals Goals Goals Goals
    </div>
  </div>
</div>
